 ...
 * Time complexity : O(n)
 * Reference : Time Bounds for Selection, Blum, M.; Floyd, R. W.; Pratt, V. R.; Rivest, R. L.; Tarjan, R. E. (August 1973).
 *
 */
public class Partition {
    ...
    DimLabel []  componentLabels;


    // the component of.getVector() according to which the partitioning is done
    private int partitionComponent;

    public Partition(DimLabel [] componentLabels){ this.componentLabels = componentLabels;}

    public Partition(List<Item> a, int partitionComponent, DimLabel [] cl ){
        arr = a;
    ...
    */
    private boolean isGreater(int first_index, int second_index) throws Exception {
        ...
        if(getValue(first_index,k) > getValue(second_index,k))
            return_value = componentLabels[k]==DimLabel.WEIGHT ? false: true;
        else if(getValue(first_index,k) < getValue(second_index,k))
            return_value = componentLabels[k]==DimLabel.WEIGHT ? true: false;
